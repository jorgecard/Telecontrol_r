...
data_dict_1 = {
  'Tensión L1-N': {'offset': 1, 'reg': 2, 'color': '#9103A6', 'label': 'Tensión L1-N', 'unit': ' [V]', 'graphic':1, 'factor':39},
  'Tensión L2-N': {'offset': 3, 'reg': 2, 'color': '#9103A6', 'label': 'Tension L2-N', 'unit': ' [V]','graphic':1, 'factor':38},
#   'Tensión L3-N': {'offset': 5, 'reg': 2, 'color': '#9103A6', 'label': 'Tensión L3-N', 'unit': ' [V]','graphic':1,'factor':37},
#   'I-L1': {'offset': 13, 'reg': 2, 'color': '#DF8905', 'label': 'I-L1', 'unit': ' [A]','graphic':2,'factor':1},
#   'I-L2': {'offset': 15, 'reg': 2, 'color': '#DF8905', 'label': 'I-L2', 'unit': ' [A]','graphic':2,'factor':1.1},
#   'I-L3': {'offset': 17, 'reg': 2, 'color': '#DF8905', 'label': 'I-L3', 'unit': ' [A]','graphic':2,'factor':1.3},
#   'E-S': {'offset': 63, 'reg': 2, 'color': '#54548D', 'label': 'E-S', 'unit': ' [VA]','graphic':3,'factor':72},
#   'E-P': {'offset': 65, 'reg': 2, 'color': '#558D54', 'label': 'E-P', 'unit': ' [W]','graphic':3,'factor':70},
#   'E-Q': {'offset': 63, 'reg': 2, 'color': '#547E8D', 'label': 'E-Q', 'unit': ' [VAR]','graphic':3,'factor':71},
#   'THD-V1': {'offset': 261, 'reg': 2, 'color': '#ADD8E6', 'label': 'THD-V1', 'unit': ' [%]','graphic':4,'factor':2.12},
...
#   'Energía Aparente': {'offset': 2817, 'reg': 2, '#EF2110': '-w', 'label': 'Energía Aparente', 'unit': ' [kVAh]','graphic':5,'factor':1}
}
...
class PyShine_LIVE_PLOT_APP(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.samplerate = 10  # tasa de muestreo a 20 Hz (estándar)
        ...
        self.plotdata = np.zeros((length, len(data_dict_1)))
    ...
    def getData(self):
        try:
            values = []
            for key, val in data_dict_1.items():
    ...
    def update_plot(self):
        try:
            data = np.zeros((1, len(data_dict_1)))
            ...
            while True:
                ...
                data = np.array(data_list).reshape(1, len(data_dict_1))
                print(data)
                # data = np.hstack((data, data + 0.3))
                # data = np.hstack((data_1, data_3))
                shift = len(data)
            ...
            self.canvas.axes.set_ylim(ymin=-0.5, ymax=240)
            # self.canvas.axes.set_ylim(ymin=120, ymax=130)
