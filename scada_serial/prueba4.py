...
dic_equipment = {
    ...
    'Inversor': {'Name': 'Inversor', 'ip': '192.168.222.180', 'port': 502, 'timeout':3,
...
data_dict_1 = {
  ...
  'Tensión L2-N': {'offset': 3, 'reg': 2, 'color': '#FF5733', 'label': 'Tension L2-N', 'unit': ' [V]','graphic':1, 'factor':1},
...
class PyShine_LIVE_PLOT_APP(QtWidgets.QMainWindow):
    ...
    def getData(self):
        try:
            ...
            for key, val in data_dict_1.items():
                ...
                raw_values = client.read_holding_registers(address=offset, count=count, unit=1)
    ...
    #                 data_1, data_3, data_4 = data_tuple[0], data_tuple[1], data_tuple[2]  # Desempaquetar los datos de la tupla
    #                 print(f'{data_1}, {data_3}, {data_4}')
                    
    #             except queue.Empty:
    #                 break
    #             # data = np.hstack((data, data + 0.3))
    #             data = np.hstack((data_1, data_3, data_4))
    #             # data = np.array([[data_1, data_3, data_4]])
    #             shift = len(data)
    ...
    #         self.canvas.axes.set_ylim(ymin=-0.5, ymax=130)
    ...
    def update_plot(self):
        try:
            # Inicializa una lista para almacenar los nuevos datos
            new_data = []
    
            while True:
                try:
                    # Obtén los datos de la cola
                    data_tuple = self.q.get_nowait()  # Obtener la tupla de la cola
                    data_1, data_3, data_4 = data_tuple[0], data_tuple[1], data_tuple[2]  # Desempaquetar los datos de la tupla
                    print(f'{data_1}, {data_3}, {data_4}')
                    
                    # Apila los nuevos datos en la lista
                    new_data.append([data_1, data_3, data_4])
                except queue.Empty:
                    break
            ...
            if new_data:
                ...
                # Desplaza los datos existentes para hacer espacio para los nuevos datos
                self.plotdata = np.roll(self.plotdata, -shift, axis=0)
                
                # Inserta los nuevos datos en la parte final de plotdata
                self.plotdata[-shift:, :] = new_data
